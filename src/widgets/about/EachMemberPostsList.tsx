...
export const EachMemberPostsList = ({ member }: { member: Member }) => {
	const {
		getMemberById,
		getTopicById,
		getPostsByContributorId,
		getPostsBySeasonId,
		filters
	} = useApp()
	...
	const filteredPosts = useMemo(() => {
		const contributorPosts = getPostsByContributorId(member.id)
	...
	}, [selectedSeasonId, member.id, getPostsByContributorId, getPostsBySeasonId])
	...
	return (
		<ul className="grid sm:grid-cols-2 lg:grid-cols-3 pb-16">
			{filteredPosts.map((post, index) => {
				const author = getMemberById(post.writerId)
